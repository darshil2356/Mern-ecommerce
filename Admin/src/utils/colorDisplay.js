...
export const isHexColor = (value = "") => /^#([0-9a-f]{3}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(String(value).trim());
...
export const getColorSwatch = (colorInput) => {
  const rawValue = typeof colorInput === "string"
    ? colorInput
    : colorInput?.hex || colorInput?.title || colorInput?.name || colorInput?.label || "";

  if (!rawValue) return "#d1d5db";
  return normalizeHex(rawValue) || "#d1d5db";
};

export const getReadableColorName = (colorInput) => {
  const rawValue = typeof colorInput === "string"
    ? colorInput
