...
const getMonthlyReport = asyncHandler(async (req, res) => {
  ...
  const orders = await Order.find({
    ...
    .populate({
      ...
      select: "title brand price barcode"
    });
  ...
  const netRevenue = orders.reduce((sum, order) => sum + (order.totalPriceAfterDiscount || 0), 0);
  ...
  const onlineSales = onlineOrders.reduce((sum, order) => sum + (order.totalPriceAfterDiscount || 0), 0);
  const offlineSales = offlineOrders.reduce((sum, order) => sum + (order.totalPriceAfterDiscount || 0), 0);
  ...
  orders.forEach(order => {
    ...
    if (!dailyBreakdown[day]) {
      dailyBreakdown[day] = { orders: 0, amount: 0 };
    }
...
const getYearlyReport = asyncHandler(async (req, res) => {
  ...
  // Get all orders for the year for detailed reporting
  const orders = await Order.find({
    createdAt: { $gte: startDate, $lte: endDate }
  })
    .populate("user", "firstname lastname mobile email")
  ...
  const monthlyData = await Order.aggregate([
    ...
    },
    {
      $group: {
        ...
        netRevenue: { $sum: "$totalPriceAfterDiscount" }
      }
    },
    { $sort: { _id: 1 } }
  ]);
  ...
  const monthlyBreakdown = monthNames.map((name, index) => {
    ...
    return {
      ...
      discount: monthData?.totalDiscount || 0,
      revenue: monthData?.netRevenue || 0
    };
  });
...
const getDateRangeReport = asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    modeBreakdown: {
      online: {
        orders: onlineOrders.length,
        amount: onlineOrders.reduce((sum, o) => sum + (o.totalPriceAfterDiscount || 0), 0)
      },
      offline: {
        orders: offlineOrders.length,
        amount: offlineOrders.reduce((sum, o) => sum + (o.totalPriceAfterDiscount || 0), 0)
      }
    },
    orders: orders.map(order => ({
      _id: order._id,
...
// Get GST report for tax compliance
const getGSTReport = asyncHandler(async (req, res) => {
  ...
  // Calculate GST (assuming 18% GST - can be customized)
  const cgstRate = 9; // CGST rate
  const sgstRate = 9; // SGST rate
  const igstRate = 18; // IGST rate
...
const getProductWiseReport = asyncHandler(async (req, res) => {
  ...
  const orders = await Order.find({
    ...
    path: "orderItems.product",
    select: "title brand price barcode category"
  });
  ...
  Object.values(productStats).forEach(p => {
    p.avgPrice = p.quantitySold > 0 ? p.totalRevenue / p.quantitySold : 0;
  });
...
const getCustomerWiseReport = asyncHandler(async (req, res) => {
  ...
  orders.forEach(order => {
    ...
    if (!customerId) {
      walkInOrders += 1;
      walkInRevenue += order.totalPriceAfterDiscount || 0;
      return;
    }
    ...
    if (!customerStats[customerId]) {
      customerStats[customerId] = {
        ...
        totalPurchase: 0,
        avgOrderValue: 0,
        firstPurchase: order.user.createdAt,
  ...
  Object.values(customerStats).forEach(c => {
    c.avgOrderValue = c.totalOrders > 0 ? c.totalPurchase / c.totalOrders : 0;
  });
