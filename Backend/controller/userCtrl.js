...
// Generate name-based referral code: e.g. JOHN123
const makeCode = (name) => {
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  let newReferralCode = makeCode(req.body.firstname);
  let existingCodeUser = await User.findOne({ referralCode: newReferralCode });
  while (existingCodeUser) {
    newReferralCode = makeCode(req.body.firstname);
    existingCodeUser = await User.findOne({ referralCode: newReferralCode });
  }
...
const createUser = asyncHandler(async (req, res) => {
  ...
  // Validate custom referral code uniqueness
  if (customReferralCode) {
    const existing = await User.findOne({ referralCode: customReferralCode.toUpperCase() });
    if (existing) {
      res.status(400);
      throw new Error(`Referral code "${customReferralCode}" is already assigned to another customer`);
    }
  }

  // Generate name-based referral code if not provided: e.g. JOHN123
  const makeCode = (name) => {
    const prefix = (name || 'USER').replace(/[^A-Z]/gi, '').toUpperCase().slice(0, 4);
    const suffix = Math.floor(100 + Math.random() * 900);
    return `${prefix}${suffix}`;
  ...
  let referralCode = customReferralCode ? customReferralCode.toUpperCase() : makeCode(firstname);
  if (!customReferralCode) {
    let existingCodeUser = await User.findOne({ referralCode });
    while (existingCodeUser) {
      referralCode = makeCode(firstname);
      existingCodeUser = await User.findOne({ referralCode });
    }
  }

  // Resolve referredBy from mobile
  let referredBy = undefined;
  if (referredByMobile) {
    const referrer = await User.findOne({ mobile: referredByMobile, role: "user" });
    if (!referrer) { res.status(404); throw new Error("Referrer with that mobile not found"); }
...
const loginUserCtrl = asyncHandler(async (req, res) => {
  ...
  if (!findUser.referralCode) {
    ...
    let referralCode = makeCode(findUser.firstname);
    let existingUser = await User.findOne({ referralCode });
    while (existingUser) {
      referralCode = makeCode(findUser.firstname);
...
const updateSettings = asyncHandler(async (req, res) => {
  ...
  const {
    ...
    storeWhatsapp, storeOpeningHours, googleMapsUrl, googleBusinessProfileUrl,
    instagramUrl, facebookUrl, youtubeUrl, storeLogo, storeFavicon, socialShareImage,
    googleReviewUrl, googleReviewRequestMessage, homepageMetaTitle, homepageMetaDescription,
    heroBannerImage, heroBannerTitle, heroBannerSubtext, heroBannerCta,
  ...
  const updatedUser = await User.findByIdAndUpdate(
    _id,
    {
      ...
      ...(storeWhatsapp !== undefined && { storeWhatsapp }),
      ...(storeOpeningHours !== undefined && { storeOpeningHours }),
      ...(googleMapsUrl !== undefined && { googleMapsUrl }),
      ...(googleBusinessProfileUrl !== undefined && { googleBusinessProfileUrl }),
      ...(instagramUrl !== undefined && { instagramUrl }),
      ...(facebookUrl !== undefined && { facebookUrl }),
      ...(youtubeUrl !== undefined && { youtubeUrl }),
      ...(storeLogo !== undefined && { storeLogo }),
      ...(storeFavicon !== undefined && { storeFavicon }),
      ...(socialShareImage !== undefined && { socialShareImage }),
      ...(googleReviewUrl !== undefined && { googleReviewUrl }),
      ...(googleReviewRequestMessage !== undefined && { googleReviewRequestMessage }),
      ...(homepageMetaTitle !== undefined && { homepageMetaTitle }),
      ...(homepageMetaDescription !== undefined && { homepageMetaDescription }),
      ...(heroBannerImage !== undefined && { heroBannerImage }),
      ...(heroBannerTitle !== undefined && { heroBannerTitle }),
      ...(heroBannerSubtext !== undefined && { heroBannerSubtext }),
      ...(heroBannerCta !== undefined && { heroBannerCta }),
      ...(promoBannerImage !== undefined && { promoBannerImage }),
      ...(promoBannerLink !== undefined && { promoBannerLink }),
      ...(homepageSectionsOrder !== undefined && { homepageSectionsOrder }),
      ...(homepageHiddenSections !== undefined && { homepageHiddenSections }),
      ...(storeFaqsJson !== undefined && { storeFaqsJson }),
  ...
  res.json({
    ...
    storeEmail: updatedUser.storeEmail,
    storeWhatsapp: updatedUser.storeWhatsapp,
    storeOpeningHours: updatedUser.storeOpeningHours,
    googleMapsUrl: updatedUser.googleMapsUrl,
    googleBusinessProfileUrl: updatedUser.googleBusinessProfileUrl,
    ...
    storeFavicon: updatedUser.storeFavicon,
    socialShareImage: updatedUser.socialShareImage,
    googleReviewUrl: updatedUser.googleReviewUrl,
    googleReviewRequestMessage: updatedUser.googleReviewRequestMessage,
    homepageMetaTitle: updatedUser.homepageMetaTitle,
    homepageMetaDescription: updatedUser.homepageMetaDescription,
    heroBannerImage: updatedUser.heroBannerImage,
    heroBannerTitle: updatedUser.heroBannerTitle,
    heroBannerSubtext: updatedUser.heroBannerSubtext,
    heroBannerCta: updatedUser.heroBannerCta,
    promoBannerImage: updatedUser.promoBannerImage,
    promoBannerLink: updatedUser.promoBannerLink,
    homepageSectionsOrder: updatedUser.homepageSectionsOrder,
    homepageHiddenSections: updatedUser.homepageHiddenSections,
    storeFaqsJson: updatedUser.storeFaqsJson,
  });
});
...
const getMyReferrals = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!user.referralCode) {
      ...
      let referralCode = makeCode(user.firstname);
      let existingUser = await User.findOne({ referralCode });
      while (existingUser) {
        referralCode = makeCode(user.firstname);
        existingUser = await User.findOne({ referralCode });
      }
      user.referralCode = referralCode;
      await user.save();
...
const updateCustomerById = asyncHandler(async (req, res) => {
  const { id } = req.params;
  validateMongoDbId(id);
  const { firstname, lastname, email, mobile, address, referralCode, referredByMobile } = req.body;

  // Validate custom referral code uniqueness
  if (referralCode) {
    const existing = await User.findOne({ referralCode, _id: { $ne: id } });
    if (existing) {
      res.status(400);
      throw new Error(`Referral code "${referralCode}" is already assigned to another customer`);
    }
  }

  // Resolve referredBy from mobile
  let referredBy = undefined;
  if (referredByMobile !== undefined) {
    if (referredByMobile === "" || referredByMobile === null) {
      referredBy = null;
    } else {
      const referrer = await User.findOne({ mobile: referredByMobile, role: "user", _id: { $ne: id } });
      if (!referrer) { res.status(404); throw new Error("Referrer with that mobile not found"); }
      referredBy = referrer._id;
    }
  }

  const updateData = { firstname, lastname, email, mobile, address };
  if (referralCode) updateData.referralCode = referralCode.toUpperCase();
